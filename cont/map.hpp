...
/*   Updated: 2022/01/01 23:50:28 by kimkwanho        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace						ft
{
	template					<class key, class typ, class cmp=std::less<key>, class alc = std::allocator<std::pair<const key, typ> > >
	class						map
	{
		public:
		...
		private:
			...
			node				_new_node(key_type _key, mapped_type _val, node _par, bool _end = false)
			{
				...
				_tmp->pair = std::make_pair(_key, _val);
				_tmp->_left = 0;
				_tmp->_right = 0;
				_tmp->_parent = _par;
				_tmp->_end = _end;
		...
		public:
			...
			mapped_type&		operator[](const key_type& _key)
			{
				...
				if (_idx != end())
					return		(_idx->second);
				return			(insert(std::make_pair(_key, mapped_type())).first->second);
			};
								...
								insert(const value_type &_val)
			{
				...
				if ((_tmp = find(_val.first)) != end())
					return		(std::make_pair(_tmp, false));
				++_len;
				return			(std::make_pair(iterator(_insert_node(_rot, _val.first, _val.second)), true));
			};
			...
			{
				return			(std::pair<const_iterator, const_iterator>(this->lower_bound(_key), this->upper_bound(_key)));
			};
