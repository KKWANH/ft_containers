	...
	*/
	...
	class 					rev_list_iterator
	{
		...
		public:
			...
			rev_list_iterator&
						operator=(const rev_list_iterator& assign)
			{
				if (this != &assign)
					_node = assign._node;
				return (*this);
		...
		/* ---------------------- OPERATOR OVERLOADS ------------------- */
			...
			rev_list_iterator&
						operator++()		{ _node = _node->prev; return (*this); }
			rev_list_iterator
						operator++(int)		{ rev_list_iterator res(*this); ++(*this); return (res); };
			rev_list_iterator&
						operator--()		{ _node = _node->next; return (*this); }
			rev_list_iterator
						operator--(int)		{ rev_list_iterator res(*this); --(*this); return (res); };
